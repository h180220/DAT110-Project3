...
public class FileManager
{
	...
	public void createReplicaFiles()
	{
		...
		{ // set a loop where size = numReplicas
			...
			replicafiles[i] = Hash.hashOf(replicafile); // hash the replica, and store the hash in the replicafiles
														// array.
		}
	}
	...
	public int distributeReplicastoPeers() throws RemoteException
	{
		...
		for (int i = 0; i < numReplicas; i++)
		{
			...
			NodeInterface succOfFile = chordnode.findSuccessor(replicafiles[i]);

			// call the addKey on the successor and add the replica
			succOfFile.addKey(replicafiles[i]);

			// call the saveFileContent() on the successor
			if (counter == index)
			{
				succOfFile.saveFileContent(filename, replicafiles[i], bytesOfFile, true);
			} else
			{
				succOfFile.saveFileContent(filename, replicafiles[i], bytesOfFile, false);
			}
